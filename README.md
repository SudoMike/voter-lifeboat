...
shows the contests Voter Lifeboat can cover for that ballot context. King
County is the first fully supported county. Unsupported Washington counties get
a statewide-only guide because the statewide package is complete for this
active election.
...
data/washington-state/counties/king/  King County local package
...
- **Coverage-aware.** Reports are marked `full_county`, `partial_county`, or
  `statewide_only`, and the Ballot Brief exports only the Covered Ballot.
...
Other pipeline stages are documented in `data/washington-state/README.md`.
